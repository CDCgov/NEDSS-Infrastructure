...
def process_hl7_segments_for_obr(
    ...
    for segment in segments:
        ...
        # Check if the segment starts with 'OBR'
        if segment_stripped.startswith('OBR'):
            # If we've collected segments for a previous OBR, write that message
        ...
        # Check if the segment starts with a known base segment prefix (MSH, PID, ORC)
        elif any(segment_stripped.startswith(prefix) for prefix in HL7_BASE_SEGMENTS_PREFIXES):
            # If we hit a new base segment, any pending OBR group is treated as complete
            if obr_related_segments:
                write_hl7_message_to_s3(
                    s3_client,
                    s3_bucket_name,
